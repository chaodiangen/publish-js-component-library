...
animate(document.getElementById('div'), {
    left: 300,
    top: 300
})
// 24、弹性运动
...
addEvent(document.querySelector('input'), 'input', vel)
// 40、节流 事件持续触发时以一定的时间间隔去定时执行回调。
...
// 诉浏览器——你希望执行一个动画，并且要求浏览器在下次重绘之前调用指定的回调函数更新动画
...
// 57、获取的是页面向上或者向左卷曲出去的距离的值,返回的是对象
function getScroll () {
    return {
